    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (rowsAffected > 0) {
                ...
                setSuccessMessage(request, "Your message has been sent successfully. We will get back to you soon!");
            } else {
                // Failed to save message
                setErrorMessage(request, "Failed to send message. Please try again later.");
            }

        } catch (SQLException e) {
            e.printStackTrace();
            setErrorMessage(request, "An error occurred while processing your request. Please try again later.");
        } finally {
            try {
                if (pstmt != null)
                    pstmt.close();
